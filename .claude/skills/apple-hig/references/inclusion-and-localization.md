...
- **Be approachable.** Require no prior skill; present a clear, straightforward interface that fits the platform, and add onboarding that lets newcomers go step by step while others skip to the content they want.
- **Avoid unnecessary references to specific genders.** "You can let a subscriber post his or her recipes to your shared folder" becomes "Subscribers can post recipes to your shared folder", which also stays inclusive in languages with gendered pronouns. Avoid gendered avatars, emoji, glyphs, and game characters; give people tools to customize them.
...
- **Ask for gender only when you require it** (health or legal reasons), with options such as _nonbinary_, _self-identify_, and _decline to state_, and consider letting people specify their pronouns.
...
- **Review the settings and objects you show.** High affluence can feel out of touch; prefer places, homes, activities, and items relatable to most people.
- **Find hidden assumptions.** A family account-access app that defines _family_ as a woman, a man, and their biological children excludes everyone else. Security questions like favorite college subject, first car, or first rainbow assume experiences not everyone shares; prefer universal ones such as favorite activity, first friend's name, or the quality that best describes you.
- **Support Apple's accessibility features** (VoiceOver, Display Accommodations, closed captioning, Switch Control, Speak Screen), and never assume a disability prevents someone from wanting the experience. Each disability is a spectrum (low vision to blindness, color blindness, blurry vision, light sensitivity, peripheral vision loss), and everyone experiences disability with age, temporarily (hearing loss from an infection), or situationally (a noisy train).
...
<anti_patterns>
- Don't frame inclusion as merely a search for offensive content.
- Don't refer to people as _the user_ or _the player_, or use _we_ for anything but the software or company.
...
- **Align a paragraph (three or more lines) based on its language, not the context.** Right-aligning LTR text hides the start of each line; one- and two-line blocks follow the context.
- **Use a consistent alignment for all items in a list,** including items in a different script.
...
- **Reverse numerals that show progress or a counting direction; never flip the numerals themselves.** Applies to numerals on progress bars, sliders, and rating controls, and to any sequence that communicates order.
- **Flip controls that show progress from one value to another** (sliders, progress indicators), reversing the glyphs or images marking the beginning and ending values.
...
- **Preserve the direction of a control that refers to an actual direction or points to an onscreen area.** "To the right" always points right.
...
- **In general, avoid flipping icons that depict real-world objects.** Clocks work the same everywhere; tools slanted for right-handed use needn't flip, since most people are right-handed.
...
- Generic person: nongendered SF Symbol; customizable avatars, emoji, and characters. Collecting gender: only when required, with _nonbinary_, _self-identify_, _decline to state_, and optional pronouns.
- Security questions and examples: universal experiences. Imagery: varied race, body type, age, and capability, including disabilities; no stereotyped roles, narrow families, or affluence.
- Color as a signal: confirm its meaning in every locale. New markets: internationalize, then localize; SF Symbols for language-specific and bidirectional glyphs.
...
- [ ] RTL sliders, progress indicators, back, next, and previous flip, while literal-direction controls, photos, logos, marks, and real objects don't?
- [ ] Text-bearing icons are localized, motion and text icons use RTL variants, and Arabic or Hebrew beside all-caps Latin is about 2 pt larger?
</quick_checklist>
