<overview>
Menus and actions on Apple platforms. Distills the HIG pages activity-views, buttons, context-menus, dock-menus, edit-menus, home-screen-quick-actions, menus, ornaments, pop-up-buttons, pull-down-buttons, the-menu-bar, and toolbars. Load this file when designing or reviewing any button, menu, toolbar, or share sheet, when deciding where a command should live (menu bar, toolbar, context menu, edit menu, Dock menu, quick action), or when laying out macOS and iPadOS menu bar menus. Toggles, segmented controls, action sheets, and keyboard shortcuts live in other reference files.
</overview>
...
An activity view (share sheet) presents sharing activities, actions such as Copy and Print, and frequently used apps for the current context, as a sheet or popover depending on device and orientation. iOS, iPadOS, visionOS only; macOS has no activity view but supports share and action app extensions.

<best_practices>
- **Don't duplicate common actions the activity view already offers.** Give similar app-specific functionality a distinct title ("Print Transaction").
- **Consider an SF Symbol for a custom activity;** center a custom interface icon in about 70x70 pixels.
- **Write a succinct verb or verb-phrase title for each custom action;** long titles wrap and truncate. A share activity's title (typically a company name) appears below its icon.
- **Exclude inapplicable system tasks (such as Print) and show custom tasks conditionally;** system tasks can't be reordered. App-specific actions list before multi-app and system actions; people can edit the list.
- **Use the Share button to display an activity view.**
- **Extensions.** Share extensions send content to apps, social accounts, and services; action extensions run content-specific tasks (add a bookmark, copy a link, edit an inline image, translate text) in place. iOS and iPadOS show both in the share sheet; macOS reaches share extensions via a toolbar Share button or a context menu Share item, and action extensions by hovering embedded content, a toolbar button, or a Finder quick action.
- **Prefer the system composition view for a share extension; include the app name in an action extension** and echo the app's interface in any custom UI.
- **Streamline to a few steps,** such as posting with a single tap.
- **A share extension uses the app icon automatically; give an action extension a symbol or icon that identifies the task.**
- **Continue lengthy work in the background and show status in the main app;** the activity view dismisses as soon as the task completes.
</best_practices>
...
A button initiates an instantaneous action and combines style (size, color, shape), content (symbol or icon, text label, or both), and role (system-defined meaning). Toggles, pop-up buttons, and segmented controls are related button-like components. (2025: updated for Liquid Glass.)

<best_practices>
- **Give every button a hit region of at least 44x44 pt, 60x60 pt in visionOS,** plus enough surrounding space, for fingertip, pointer, eyes, or remote.
- **Always include a press state for a custom button.**
- **Use a prominent style for the most likely action in a view;** the system applies the accent color to its background. One or two prominent buttons per view.
- **Use style, not size, to distinguish the preferred choice;** same-size buttons read as one coherent set.
- **Make each button's purpose clear** with a symbol, a text label, or both. macOS and visionOS show a tooltip on hover.
- **Pair familiar actions with familiar icons** (`square.and.arrow.up` means share) from SF Symbols and the Standard icons.
- **Use text when a short label is clearer than an icon:** title-style capitalization, verb-led ("Add to Cart").
- **Roles:** Normal; Primary (the default button; app accent color; responds to Return and lets a sheet, editable view, or alert close on Return); Cancel; Destructive (system red). Assign Primary to the button people are most likely to choose.
</best_practices>

<platform_considerations>
- **iOS, iPadOS:** For an action that doesn't complete instantly, configure the button to show an activity indicator, optionally with an alternate label ("Checkout" to "Checking out…"); the indicator appears beside the label and hides the button image.
...
- Avoid a similar color on button labels and content layer backgrounds; with colorful content prefer the default monochromatic labels (2025, Liquid Glass color).
- Don't assign the primary role to a destructive button, even when it is the likely choice.
...
A context menu (contextual menu on macOS) gives access to functionality directly related to an item without cluttering the interface. Hidden by default; revealed by touch or pinch and hold (iOS, iPadOS, visionOS), Control-click (macOS, iPadOS), or a secondary click on a Magic Trackpad. Not supported in watchOS.

<best_practices>
- **Prioritize relevancy:** the commands people most likely need now, not advanced or rare ones (Mail's Inbox message menu: reply and move, not editing or mailbox management).
- **Aim for a small number of items.**
- **Support context menus consistently throughout the app.**
- **Always make context menu items available in the main interface too** (a toolbar in iOS and iPadOS; the menu bar in macOS).
- **Keep submenus to one level** with titles that predict contents.
- **Hide unavailable items, don't dim them.** macOS exception: Cut, Copy, and Paste may appear unavailable.
- **Put the most frequently used items nearest the finger or pointer;** reverse the order when the menu opens above the content.
- **Show keyboard shortcuts in the app's main menus, not in context menus.**
- **Use separators; keep to about three groups.**
- **iOS, iPadOS, visionOS: mark destructive items (Delete, Remove) as `destructive` and list them last;** the system can render them in red.
- **Content:** seldom a title; add one only when it clarifies effect (Mail's Mark menu shows the count of selected messages). Short descriptive labels, and the system's icons for Copy, Share, Delete, and similar actions.
</best_practices>

<platform_considerations>
- **iOS, iPadOS:** Provide either a context menu or an edit menu for an item, never both. In iPadOS consider a context menu for creating a new object (Files creates a folder from empty space). A context menu can show a preview that people may tap to open or drag; prefer a graphical preview that confirms the target (condensed content in Notes and Mail), and match its clipping path to the image so rounded corners don't appear to change during the emergence animation (`UIContextMenuInteractionDelegate`).
- **visionOS:** Consider a context menu instead of a panel or inspector window for frequently used functionality. Keep the menu shorter than the window so it doesn't obscure window-management controls or the Share menu; size the item count to the audience.
</platform_considerations>
...
<best_practices>
- **Label items succinctly and organize them logically** per Menus.
- **Make custom Dock menu items available elsewhere too,** in menu bar menus or the interface.
- **Prefer high-value custom items:** open or recent windows, plus a few actions useful when the app isn't frontmost or has no windows (Mail: get new mail, compose).
</best_practices>
...
- **Offer only commands relevant in context; remove or dim the rest.**
...
- **Appearance:** iOS shows a compact horizontal list on touch and hold or double-tap, with a trailing chevron that expands it into a context menu; iPadOS uses the horizontal style for touch and opens as a context menu for keyboard or pointer; macOS offers editing commands in a context menu and the menu bar Edit menu; visionOS opens a horizontal bar on pinch and hold, or a context menu.
</best_practices>

<platform_considerations>
- **iOS, iPadOS:** Make the menu work in both styles: compact horizontal for Multi-Touch, vertical for keyboard or pointing device (see the Menus layouts). Default position is above or below the insertion point or selection with a pointer indicator; shape and pointer are fixed, but the menu can be moved to avoid covering important content.
...
- Avoid Copy or Cut with no selection, or Paste with nothing to paste.
...
A menu reveals its options on interaction; each menu item is a command, option, or state affecting the current selection or context. These label and organization rules apply to every menu type: pop-up, pull-down, context, menu bar, Dock, in-game. (2026: menu item icon guidance updated.)
...
- **Dim unavailable items.** If all items are unavailable, keep the menu openable so people can learn its commands.
- **Append an ellipsis (…) when an action needs more input** before it completes.
- **Represent common actions with the system's standard icons (2026):** Share, Print, Search, and similar.
...
- **Give icons to all items in a group or to none (2026).**
- **List important or frequently used items first.**
- **Group related items with separators** (a line or a short gap, by platform and menu type), keeping related commands together even at different frequencies (Paste and Match Style with Copy, Cut, and Paste).
...
<best_practices>
- **Provide a useful default selection;** the button shows it until people choose.
...
<platform_considerations>
- **iPadOS:** In a popover or modal view, consider a pop-up button instead of a disclosure indicator for a list item with a small, well-defined set of options.
</platform_considerations>
...
- **App menu (name in bold):** About _AppName_ (first, alone in its group; short name of 16 characters or fewer; no version number); Settings… (app-level only; document-specific settings go in File; on iPadOS opens the app's page in Settings); optional app-configuration items after Settings in the same group; Services (macOS); Hide _AppName_, Hide Others, Show All (macOS); Quit _AppName_ (Option: Quit and Keep Windows).
...
- **Window menu (navigate, organize, manage windows; never appearance or Close):** Minimize (Option: Minimize All); Zoom (Option: Zoom All; never for full screen); Show Previous Tab; Show Next Tab; Move Tab to New Window; Merge All Windows; Enter/Exit Full Screen (only without a View menu, keeping Minimize and Zoom separate); Bring All to Front (Option: Arrange in Front); open windows alphabetically, excluding panels and modal views. Provide it even with one window so Full Keyboard Access can Minimize and Zoom. Consider show/hide items for panels (not font or color panels, which Format covers).
...
- **Dynamic menu items (a modifier changes the item: Minimize to Minimize All with Option):** never the only way to do a task; primarily in menu bar menus; a single modifier key (`isAlternate`). macOS sizes menus to the widest item, including dynamic ones.
</best_practices>
...
- **macOS:** The toolbar lives in the window frame at the top, below or integrated with the title bar; titles can display inline with controls and items have no bezel. Make every toolbar item available as a menu bar command, because people can customize or hide the toolbar.
...
- Avoid custom toolbar backgrounds and tinted controls (2025).
- Avoid a similar color on toolbar item labels and content backgrounds; with colorful content prefer the default monochromatic appearance (2025).
